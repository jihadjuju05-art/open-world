...
export const GAITS_GLTF = { walk: 1.25, jog: 3.4, run: 5.6 };     // matched to the library clips' natural speeds (walk .98, jog 5.4, sprint 8.3 m/s)
...
export class Player {
  ...
  // Locomotion for the glTF character: gait chosen from the real ground speed with hysteresis, playback rate matched to each clip's natural
  ...
  gltfLoco(dt, swimming) {
    ...
    if (this.locoOnce > 0) { this.locoOnce -= dt; if (this.locoOnce <= 0) { this.locoOnce = 0; if (b.action && this.locoOwn && /^Jump_(Start|Land)$/.test(b.action.name)) b.endAction(); this.locoOwn = false; } }
    ...
    if (air && this.jumped && this.airT < .12) { b.startAction('Jump_Start', { rate: 1.9, fade: .05 }); this.locoOwn = true; this.locoOnce = .22; this.jumped = false; }
